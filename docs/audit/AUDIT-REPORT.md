...
> **⚠️ Обновление от 2026-09-04 (после даты аудита).** Строка **R1 / «Собственные боты клиента»** ниже помечена как «Не реализовано, 0%» — это устарело. Блок был реализован в `01a533b`/`a0568cd` (24.07.2026), **после** даты аудита (20.07.2026), и с тех пор доработан: сохранение и проверка токена клиента, приём `/start` с меткой, дедуп повторного старта, отслеживание блокировки/разблокировки бота, отдельная привязка «Особистий акаунт» (без доступа к переписке), активное Telegram-уведомление владельцу при отзыве токена, отчёт `/reports/bot-starts` со сводкой «кліки → старти». Из заявленного в R1 по-прежнему отсутствует только **deep-goal REST API** (произвольные конверсии от клиентского бота, не только `/start`) и **webhook-роутинг** (сейчас клиентские боты работают через polling, не webhook). Строка R1 и соответствующая ей запись в `audit-matrix.csv`/`.json` исправлены на актуальное состояние; **остальные цифры документа (executive summary, ~35%, разбивка по 91 функции, P1-список) не пересчитывались** и остаются на дату аудита — расхождение в одну строку не двигает их заметно.
...
| R. Свои боты клиента | ~65% (обновлено 04.09, см. врезку выше) | Подключение токена, `/start`-атрибуция, блок/анблок, отчёт с кликами и стартами — реализовано; нет deep-goal API и webhook-роутинга (только polling) |
...
1. ~~**Свои боты клиента (BotConnection)** (XL) — активировать существующую модель `TelegramBotConnection`: сохранение токена, per-bot webhook роутинг, health-check, deep goal REST API, HMAC, rate limit, OpenAPI.~~ Базовая часть сделана (см. врезку в начале документа): сохранение и проверка токена, polling, `/start`-атрибуция, health-статус, уведомление владельца при отзыве токена. Осталось: per-bot **webhook**-роутинг вместо polling (S); deep goal REST API — см. пункт 2 ниже, это тот же незакрытый кусок.
